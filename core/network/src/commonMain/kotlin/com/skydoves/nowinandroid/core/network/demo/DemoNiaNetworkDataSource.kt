...
class DemoNiaNetworkDataSource(
  ...
  override suspend fun getNewsResources(
    ids: List<String>?
  ): ApiResponse<List<NetworkNewsResource>> = readAsset(NEWS_ASSET)
  ...
  private suspend inline fun <reified T> readAsset(fileName: String): ApiResponse<List<T>> =
    withContext(ioDispatcher) {
      try {
        ApiResponse.Success(
          networkJson.decodeFromString<List<T>>(Res.readBytes("files/$fileName").decodeToString())
        )
 ...
 */
private fun <T> ApiResponse<List<T>>.mapToChangeList(
  idGetter: (T) -> String
): ApiResponse<List<NetworkChangeList>> =
  when (this) {
